...
const DEFAULT_HIGHLIGHTER_TEMPLATE = `TASK:
You'll find below a STATEMENT and a QUESTION related to it.
The STATEMENT is a plea from a road offender.
The QUESTION is from a magistrate trying to quickly find clues in the STATEMENT.
Generate a response in valid JSON. 
...
* \`highlights\`: an array of relevant highlight objects extracted from the STATEMENT; 
* \`reasoning\`: one sentence to summarise the reasoning to answer the QUESTION from the highlights;
* \`answer\`: the actual categorical answer (e.g. yes, no, true, false, ...) in just one or a few words

Only answer if confident based on the evidence from the highlights. 
Otherwise, answer "undetermined" and explain why in the "reasoning" property.
...
const SETTINGS = {
  ...
  questions: {
    default: `Is there a reason why the defendant might not be guilty?
      Is there a reason this case should be heard in open court?`,
    type: 'textarea',
  },
  model: {
    default: "qwen3:30b",
  ...
  serviceUrl: {
    ...
    inQueryString: true,
    lookup: ['CACHED', 'http://localhost:11434/v1', 'http://localhost:11436/v1', "https://ai.create" + ".kcl.ac.uk/api/", "https://openrouter.ai/api/v1"],
  },
  apiKey: {
    default: '',
    type: 'password',
  ...
  // },
  templateHighlighter: {
    default: DEFAULT_HIGHLIGHTER_TEMPLATE,
    type: 'textarea',
  },
